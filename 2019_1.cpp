...
public:
    ...
    Dictionary<T1, T2> getRange(int a, int b) const {
        if (a < 0 || b >= *_trenutno || a > b || b < 0) {
            throw exception("Unijeli ste nemoguc interval");
        }
        Dictionary<T1, T2> novi;
        for (int i = a; i <= b; i++) {
...
class DatumVrijeme {
    ...
    int toMinute() const {
        return *_godina * 365 * 24 * 60 * 60 + *_mjesec * 30 * 24 * 60 * 60 + *_dan * 24 * 60 * 60 + *_sati * 60 + *_minuti;
    }
public:
    ...
    friend bool prosao(const DatumVrijeme& d1, const DatumVrijeme& d2) {
...
class Student {
    ...
    bool validanEmail(string s) {
        return regex_match(s, regex("\\w*\\.?\\w*\\@(hotmail.com|outlook.com|fit.ba)"));
    }
    bool validanBrojTelefona(string s) {
        return regex_match(s, regex("\\d{3}\\s\\d{3}\\s\\d{3}"));
    }
    ...
    void PosaljiEmail(const GodinaStudija& g, double prosjek) {
        ...
        cout << "TO: emailStudenta" << endl;
        cout << "Postovani " << _imePrezime << ", evidentirali ste uspjeh za " << g << "godinu studija." << endl;
        cout << "Pozdrav." << endl;
        cout << "FIT Team." << endl;
        m.unlock();
        if (prosjek > 8.0) {
...
public:
    Student(const char* imePrezime, string emailAdresa, string brojTelefona) {
        ...
        if (validanBrojTelefona(_brojTelefona)) {
            _brojTelefona = brojTelefona;
        }
        else {
            _brojTelefona = "000 000 000";
        }
    }
    ...
    bool AddPredmet(const Predmet& p, const GodinaStudija& g, const DatumVrijeme& d) {
        for (auto i = _uspjeh.begin(); i != _uspjeh.end(); i++) {
            if (*i->GetGodinaStudija() == g) {
                ...
                if (!prosao(d, i->GetPredmeti()->getElement2(i->GetPredmeti()->getTrenutno() - 1)))
                    return false;
    ...
    int BrojPonavljanjaRijeci(string s) {
        regex rule(s);
        int suma = 0;
        for (auto i = _uspjeh.begin(); i != _uspjeh.end(); i++) {
            for (int j = 0; j < i->GetPredmeti()->getTrenutno(); j++) {
        ...
        for (auto i = _uspjeh.begin(); i != _uspjeh.end(); i++) {
            for (int j = 0; j < i->GetPredmeti()->getTrenutno(); j++) {
                if (*d1 < i->GetPredmeti()->getElement2(j) && i->GetPredmeti()->getElement2(j) < *d2)
                    novi.emplace_back(i->GetPredmeti()->getElement1(j));
            }
        }
        return novi;
    }
